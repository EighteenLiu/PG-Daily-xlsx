...
PHOTO_FIELDS = ["问题照片1", "问题照片2", "问题照片3", "问题照片4"]
...
class ExcelProcessor:
    ...
    def _write_sheet(self, sheet: Worksheet, headers: list[str], rows: Iterable[list[object]]) -> None:
        ...
        for idx, header in enumerate(headers, start=1):
            ...
            if header in FIXED_COLUMN_WIDTHS:
                sheet.column_dimensions[sheet.cell(row=1, column=idx).column_letter].width = FIXED_COLUMN_WIDTHS[header]
            else:
                sheet.column_dimensions[sheet.cell(row=1, column=idx).column_letter].width = min(max(max_len + 2, 10), 35)
    ...
    def _display_header_cell(self, header: str) -> tuple[str, str]:
        ...
        if header in INDICATOR_FIELDS:
            return "指标", header
        if header in PHOTO_FIELDS:
            return "问题照片", f"图片{PHOTO_FIELDS.index(header) + 1}"
        if header == "整改照片":
            return "整改照片", "图片1"
        if header == "二维码":
            return "二维码", "图片1"
    ...
    def _move_photos_to_tail(
        ...
        existing = [h for h in PHOTO_FIELDS if h in headers]
        if not existing:
            return headers, rows
    ...
    def extract_date_label(self, filename: str) -> str:
        ...
        match = re.search(r"(\d{1,2})月(\d{1,2})日", stem)
        if match:
            return f"{int(match.group(1))}月{int(match.group(2))}日"
        match = re.search(r"(20\d{2})[-_.年]?(\d{1,2})[-_.月]?(\d{1,2})日?", stem)
        if match:
